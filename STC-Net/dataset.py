...
from CopyNet.copynet.utils import contains_digit, tokens_to_seq
import torch
...
class SequencePairDataset(Dataset):
    ...
    def judge_comparative_type(self, input_tokes):
        if input_tokes[1] in gradable_candidates:
            if "comparee" in input_tokes[2:]:
                return 0
            elif "index" in input_tokes[2:]:
                return 1
            elif "difference" in input_tokes[2:]:
                return 2
            else:
                return 5
        elif input_tokes[1] in superlative_candidates:
            if "comparee" in input_tokes[2:]:
                return 3
            elif "index" in input_tokes[2:]:
                return 4
    ...
    def __getitem__(self, idx):
        ...
        comparee_type = self.judge_comparative_type(input_token_list)
        if self.use_cuda:
            input_seq = input_seq.to(device)
            output_seq = output_seq.to(device)

        return input_seq, output_seq, ' '.join(input_token_list), ' '.join(output_token_list),comparee_type
