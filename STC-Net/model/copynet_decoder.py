...
from CopyNet.copynet.dataset import Language
from CopyNet.copynet.utils import DecoderBase, to_one_hot
...
class CopyNetDecoder(DecoderBase):
    ...
    def forward(self, encoder_outputs, inputs, final_encoder_hidden,comparative_tensor, targets=None, keep_prob=1.0, teacher_forcing=0.0):
        ...
        for step_idx in range(1, self.max_length):
            ...
            sampled_idx, output, hidden, selective_read, scores = self.step(sampled_idx, hidden, encoder_outputs,comparative_tensor, selective_read, one_hot_input_seq, dropout_mask=dropout_mask)
            if len(attention_scores) < scores.shape[1]:
    ...
    def step(self, prev_idx, prev_hidden, encoder_outputs, comparative_tensor, prev_selective_read, one_hot_input_seq, dropout_mask=None):
        ...
        #Soft Template
        output = output + comparative_tensor
        # Generate mechanism
        ...
        attn_score = attn_weights.squeeze(dim=1)
        return sampled_idx, log_probs, hidden, selective_read, attn_score
