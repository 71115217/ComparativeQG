...
from CopyNet.copynet.dataset import SequencePairDataset
from CopyNet.copynet.evaluate import evaluate
from CopyNet.copynet.model.encoder_decoder import EncoderDecoder
from CopyNet.copynet.utils import trim_seqs, to_np
...
def train(encoder_decoder: EncoderDecoder,
    ...
    for epoch, teacher_forcing in enumerate(teacher_forcing_schedule):
        ...
        for batch_idx, (input_idxs, target_idxs, input_tokens, target_tokens) in enumerate(tqdm(train_data_loader)):
            ...
            #batch_bleu_score = corpus_bleu(batch_targets, batch_outputs,weights = (1, 0, 0, 0), smoothing_function=SmoothingFunction().method1)
            # print(batch_bleu_score)
            # if global_step < 10 or (global_step % 10 == 0 and global_step < 100) or (global_step % 100 == 0 and epoch < 2):
            #     input_string = "what  last parameter episode of season 1  title"
            #     output_string = encoder_decoder.get_response(input_string)
            #     writer.add_text('schedule', output_string, global_step=global_step)
            #
            #     input_string = "who  first parameter miami heat player  name"
            #     output_string = encoder_decoder.get_response(input_string)
