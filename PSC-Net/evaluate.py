...
from PseudoSQLCopyNet.pseudoCopynet.utils import trim_seqs, seq_to_string
from PseudoSQLCopyNet.pseudoCopynet.utils import to_np
...
def evaluate(encoder_decoder: EncoderDecoder, data_loader):
    ...
    for batch_idx, (input_idxs, target_idxs, _, _) in enumerate(tqdm(data_loader)):
        ...
        output_log_probs, output_seqs, attn_scores = encoder_decoder(input_variable, list(sorted_lengths))
        # print(attn_scores)
        # print("predict_sentence:")
        # print(trim_seqs(output_seqs))
...
def corpus_rouge(all_target_seqs, all_output_seqs):
    ...
    for index in range(len(pair_target)):
        ...
        rouge_score = rouge.get_scores(pair_target[index], pair_output[index])
        rouge_1 = rouge_score[0]["rouge-1"]['f']
        rouge_2 = rouge_score[0]["rouge-2"]['f']
        rouge_L = rouge_score[0]["rouge-l"]['f']
        # rouge_1 = Rouge_1(" ".join(decoded_words), pair[1])
        # rouge_2 = Rouge_2(" ".join(decoded_words), pair[1])
        # rouge_L = Rouge_L(" ".join(decoded_words), pair[1])
        rouge_avg_1 = rouge_avg_1 + rouge_1
        rouge_avg_2 = rouge_avg_2 + rouge_2
        rouge_avg_L = rouge_avg_L + rouge_L
    ROUGE1 = float(rouge_avg_1/ len(pair_target))
    ROUGE2 = float(rouge_avg_2 / len(pair_target))
    ROUGEL = float(rouge_avg_L / len(pair_target))
