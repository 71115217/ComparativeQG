...
from PseudoSQLCopyNet.pseudoCopynet.utils import contains_digit, tokens_to_seq
import torch
...
class SequencePairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with open(self.data_path + self.files[idx], "r", encoding='utf-8') as pair_file:
            ...
            sql_token_list = pair_file.readline().split()
            comparative_token_list = pair_file.readline().split()

        input_token_list = (['<SOS>'] + input_token_list + ['<EOS>'])[:self.maxlen]
        output_token_list = (['<SOS>'] + output_token_list + ['<EOS>'])[:self.maxlen]

        sql_token_list = (['0'] + sql_token_list + ['2'])[:self.maxlen]
        comparative_token_list = (['0'] + comparative_token_list + ['9'])[:self.maxlen]
        for i in range(self.maxlen - len(sql_token_list)):
            sql_token_list.append("3")
        for i in range(self.maxlen - len(comparative_token_list)):
            comparative_token_list.append("10")

        sql_token_list = list(map(int, sql_token_list))

        comparative_token_list = list(map(int, comparative_token_list))
        sql_tokens  = torch.tensor(sql_token_list)
        comparative_tokens = torch.tensor(comparative_token_list)
        ...
        if self.use_cuda:
            ...
            sql_tokens = sql_tokens.to(device)
            comparative_tokens = comparative_tokens.to(device)

        return input_seq, output_seq, ' '.join(input_token_list), ' '.join(output_token_list), sql_tokens, comparative_tokens
